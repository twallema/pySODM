...
def log_prior_normal_L2(x, L2_params):
    ...
    L2_params: tuple
        ...
        This implies the reguralisation weight express how confident you are about your prior beliefs
...
class log_posterior_probability():
    ...
    """
    def __init__(self, models, parameter_names, bounds, data, states, log_likelihood_fnc, log_likelihood_fnc_args,
                 start_sim=None, end_sim=None, weights=None, log_prior_prob_fnc=None, log_prior_prob_fnc_args=None, aggregation_function=None, labels=None):
        ...
        # assert length of datasets, model states, weights, log likelihood functions and arguments are equal
        if weights is None:
        ...
        # check if user wants to use the same or a different model for every dataset
        self.multiple_models = False
        if isinstance(models, list):
            assert len(models) == len(data), f'the number of provided models ({len(models)}) must match the number of datasets ({len(data)})'
        ...
        data, self.time_index, self.additional_axes_data = self._validate_dataset(data)

        #################################
        ## Validate start- and end sim ##
        #################################
        ...
        if not self.multiple_models:
            parameter_sizes, self.parameter_shapes = self._validate_calibrated_parameters(parameter_names, models.parameters)
        else:
            # get the parameter shapes
            parameter_shapes = []
            for model in self.models:
                # TODO: print an error message highlighting in what model the parameter is missing
                parameter_sizes, parshapes = self._validate_calibrated_parameters(parameter_names, model.parameters)
                parameter_shapes.append(parshapes)
            # loop over the parameters, assert parameter-per-parameter if all shapes are the same
            for parname in parshapes.keys():
                shapes = []
                for d in parameter_shapes:
                    shapes.append(d[parname])
                assert all(shape == shapes[0] for shape in shapes), f"detected a different shape for parameter '{parname}' between the provided `models`"
            # assign first element 
            self.parameter_shapes = parameter_shapes[0]
        ...
        self.parameters_names_postprocessing = self._expand_parameter_names(self.parameter_shapes)
        # Expand bounds
        if len(bounds) == len(parameter_names):
            self._check_bounds(bounds)
            self.expanded_bounds = self._expand_bounds(parameter_sizes, bounds)
        ...
        if labels:
            if len(labels) == len(parameter_names):
                self.expanded_labels = self._expand_labels(self.parameter_shapes, labels)
        ...
        self.log_prior_prob_fnc, self.log_prior_prob_fnc_args = self._validate_expand_log_prior_prob(log_prior_prob_fnc, log_prior_prob_fnc_args, parameter_sizes, self.expanded_bounds)
        ...
        if aggregation_function:
            aggregation_function = self._validate_aggregation_function(aggregation_function, len(data))
        ...
        if not self.multiple_models:
            out = self._create_fake_xarray_output(self.models.dimensions_per_state, self.models.state_coordinates, self.models.initial_states, self.time_index)
            self.coordinates_data_also_in_model, self.aggregate_over = self._compare_data_model_coordinates(out, data, states, aggregation_function, self.additional_axes_data)
        else:
        ...
        self.n_log_likelihood_extra_args = self._validate_log_likelihood_funtion(log_likelihood_fnc)
        self.log_likelihood_fnc_args = self._validate_log_likelihood_function_extra_args(data, self.n_log_likelihood_extra_args,
                                                                                            self.additional_axes_data, self.coordinates_data_also_in_model,
    ...
    def __call__(self, thetas, simulation_kwargs={}):
        ...
        # If a model for every dataset is provided, loop over models
        if not self.multiple_models:
    ...
    def _validate_dataset(data):
        ...
        assert isinstance(data, list), "'data' must be a list containing pd.Series/pd.DataFrame or xarray.DataArray/xarray.Dataset"
        ...
        if all(index == 'date' for index in time_index):
            ...
            raise ValueError(
                "Some datasets have 'time' as time index, other have 'date' as time index. pySODM does not (yet) allow mixing."
            )
        return data, time_index, additional_axes_data

    @staticmethod
    def _check_simulation_time(startend_sim, start_or_end, data, time_index):
        """
        A function to validate the start and end of the simulations
        """

        assert start_or_end in ['start_sim', 'end_sim']
        # check start_sim
        if startend_sim:
            if time_index == 'time':
                assert isinstance(startend_sim, (int,float)), f"'{start_or_end}' must be of type 'int' or 'float'"
            else:
                assert isinstance(startend_sim, (str, datetime)), f"'{start_or_end}' must of type 'str' or 'datetime'. '{type(startend_sim)}' provided."
                if isinstance(startend_sim, str):
                    try:
                        startend_sim = datetime.strptime(startend_sim,  "%Y-%m-%d")
                    except:
                        raise ValueError(f"conversion of '{start_or_end}' to datetime failed. make sure its formatted as '%Y-%m-%d'")
        else:
            if start_or_end == 'start_sim':
                startend_sim = min([df.index.get_level_values(time_index).unique().min() for df in data])
            else:
                startend_sim = max([df.index.get_level_values(time_index).unique().max() for df in data])
        return startend_sim

    @staticmethod
    def _validate_calibrated_parameters(parameters_function, parameters_model):
        """
    ...
    @staticmethod
    def _expand_parameter_names(parameter_shapes):
    ...
    @staticmethod
    def _expand_bounds(parameter_sizes, bounds):
        """"
    ...
    @staticmethod
    def _expand_labels(parameter_shapes, labels):
    ...
    @staticmethod
    def _validate_expand_log_prior_prob(log_prior_prob_fnc, log_prior_prob_fnc_args, parameter_sizes, expanded_bounds):
        """ 
    ...
    @staticmethod
    def _get_coordinates_data_also_in_model(data_index_diff, i, model_coordinates, data):
    ...
    @staticmethod
    def _get_dimensions_sum_over(additional_axes_data, model_coordinates):
    ...
    @staticmethod
    def _validate_aggregation_function(aggregation_function, n_datasets):
    ...
    @staticmethod
    def _create_fake_xarray_output(dimensions_per_state, state_coordinates, initial_states, time_index):
        """ 
    ...
    def _compare_data_model_coordinates(self, output, data, calibration_state_names, aggregation_function, additional_axes_data):
        """
        ...
        for i, (state_name, df) in enumerate(zip(calibration_state_names, data)):
            ...
            coordinates_data_also_in_model.append(self._get_coordinates_data_also_in_model(additional_axes_data[i], i, dims_coords, data))
            
            # Construct a list containing (per dataset) the axes we need to sum the model output over prior to matching the data
            aggregate_over.append(self._get_dimensions_sum_over(additional_axes_data[i], dims_coords))

        return coordinates_data_also_in_model, aggregate_over

    @staticmethod
    def _validate_log_likelihood_funtion(log_likelihood_function):
        """
    ...
    @staticmethod
    def _validate_log_likelihood_function_extra_args(data, n_log_likelihood_extra_args, additional_axes_data, coordinates_data_also_in_model, time_index, log_likelihood_fnc_args,
                                                    log_likelihood_fnc):
